...
try:
    ...
    ######################################################################################
    def job_save_game_input_history():
        global log_handler,logging
        log_handler.removeHandler(log_handler.handlers[1])
        log_handler,logging = log_config.create_logger(log_file_name)
        try:
            ...
            API_method.app_start_job('save_game_input_history',config_number = config_number)
            return {'result':1, "outString":"post successful"}
        except Exception as e:
            log_handler.exception(f"""#####################{env_name} Catch an exception.#####################""")
            logging.shutdown()
...
if __name__ == "__main__":
    ...
    scheduler.add_job(job_betradar,"cron",day_of_week = "mon-sun", hour = 20, minute = 0,misfire_grace_time=60*60)
    
    scheduler.add_job(job_save_game_input_history,"cron",day_of_week = "mon-sun", hour = 19, minute = 0,misfire_grace_time=60*60)
